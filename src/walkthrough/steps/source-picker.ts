...
export async function pickSource(): Promise<SourceDescriptor> {
  // With a single registered source there's nothing to choose — auto-select it and
  // skip a dead 1-option prompt. Stays robust if more sources are registered later.
  if (SOURCES.length === 1) {
