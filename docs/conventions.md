...
- **`helpers.ts`** — pure utility functions used only inside the feature (e.g. `pad`, `detectExtFromBytes` in `downloader/`).
...
- Colocated with the module: `src/<feature>/foo.test.ts` (e.g. `src/downloader/foo.test.ts`)
