...
Capturing your real-browser Cloudflare session lets the downloader (mangakakalot.gg) bypass the bot protection. The interactive walkthrough (`bun start`) handles this automatically, just-in-time, when Mangakakalot is selected and no valid session exists.

## Primary: undetected-browser capture (patchright)

When the session probe reports the stored session is stale or absent, the walkthrough:

1. Launches your real, installed Chrome via `patchright` (a Playwright fork with the automation-detection leaks patched out — Cloudflare sees a real, headed Chrome, not an automated one).
2. Opens the same probe URL used to validate sessions and waits for you to solve any Cloudflare challenge in the visible window.
3. Once you've solved it, harvests the fresh `cf_clearance` cookie and the exact User-Agent directly from the live browser context — no DevTools, no copying anything.
4. Validates the captured session with the existing probe and, on success, persists it to `auth.json`.

This runs at most once per run, only when the probe detects a stale/absent session. There is nothing to copy or paste for this path — just solve the Cloudflare check in the window that opens.

## Fallback: manual cURL paste

If browser capture isn't available (no Chrome installed), the user cancels, the capture errors, or the post-capture probe still fails, the walkthrough automatically falls back to the manual cURL paste flow below. No configuration is needed — the fallback is transparent.
...
> walkthrough (`bun start`) handles auth inline (browser capture, falling back to the Quick-version prompt above).
...
`cf_clearance` is valid for approximately **30 days**. The walkthrough re-captures automatically (browser capture, falling back to cURL paste) once the session probe reports it stale.
